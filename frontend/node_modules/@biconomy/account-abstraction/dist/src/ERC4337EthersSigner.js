...
class ERC4337EthersSigner extends abstract_signer_1.Signer {
    ...
    async sendTransaction(transaction, walletDeployOnly = false, paymasterServiceData, engine // EventEmitter
    ) {
        var _a, _b;
        const socketServerUrl = this.config.socketServerUrl;
        common_1.Logger.log('paymasterServiceData', paymasterServiceData);
                ...
                onError: async (tx) => {
                    if (tx) {
                        ...
                        if (err) {
                            ...
                            throw new Error('Bundler failed to send transaction. Please report with transactionId');
                        }
                    }
                }
            });
    ...
    async sendTransactionBatch(transactions, paymasterServiceData, engine // EventEmitter
    ) {
        const socketServerUrl = this.config.socketServerUrl;
        const clientMessenger = new gasless_messaging_sdk_1.ClientMessenger(socketServerUrl, isomorphic_ws_1.default);
        if (!clientMessenger.socketClient.isConnected()) {
